...
IRAM_ATTR static void _processing_uvc_pipe(_uvc_stream_handle_t *strmh, hcd_pipe_handle_t pipe_handle, bool if_enqueue)
{
    ...
    if (urb_done->transfer.num_isoc_packets == 0)
    { // Bulk transfer
...
static esp_err_t _update_config_from_descriptor(const usb_config_desc_t *cfg_desc)
{
            ...
            else if (context_class == USB_CLASS_VIDEO && context_subclass == VIDEO_SUBCLASS_STREAMING)
            {
                if (context_intf_alt == 0)
                {
                    ...
                    case VIDEO_CS_ITF_VS_FRAME_MJPEG:
                    {
                        ...
                        if (((_frame_width == usb_dev->uvc_cfg.frame_width) || (FRAME_RESOLUTION_ANY == usb_dev->uvc_cfg.frame_width)) && ((_frame_heigh == usb_dev->uvc_cfg.frame_height) || (FRAME_RESOLUTION_ANY == usb_dev->uvc_cfg.frame_height)))
                        {
    ...
    if (usb_dev->enabled[STREAM_UVC])
    {
        if (vs_intf_found)
        {
            ...
            ESP_LOGI(TAG, "\tEndpoint(%s) Addr = 0x%x, MPS = %u", uvc_dev->vs_ifc->xfer_type == UVC_XFER_ISOC ? "ISOC" : (uvc_dev->vs_ifc->xfer_type == UVC_XFER_BULK ? "BULK" : "Unknown"), vs_intf_ep_addr, vs_intf_ep_mps);
        }
    ...
    if (usb_dev->enabled[STREAM_UAC_SPK] && as_spk_intf_found && uac_ver_found == UAC_VERSION_1 && !uac_format_others_found)
    {
        ...
        ESP_LOGI(TAG, "\tEndpoint(%s) Addr = 0x%x, MPS = %u", (as_spk_intf_ep_attr & USB_BM_ATTRIBUTES_XFERTYPE_MASK) == USB_BM_ATTRIBUTES_XFER_ISOC ? "ISOC" : ((as_spk_intf_ep_attr & USB_BM_ATTRIBUTES_XFERTYPE_MASK) == USB_BM_ATTRIBUTES_XFER_BULK ? "BULK" : "Unknown"), as_spk_intf_ep_addr, as_spk_intf_ep_mps);
    ...
    if (usb_dev->enabled[STREAM_UAC_MIC] && as_mic_intf_found && uac_ver_found == UAC_VERSION_1 && !uac_format_others_found)
    {
        ...
        ESP_LOGI(TAG, "\tEndpoint(%s) Addr = 0x%x, MPS = %u", (as_mic_intf_ep_attr & USB_BM_ATTRIBUTES_XFERTYPE_MASK) == USB_BM_ATTRIBUTES_XFER_ISOC ? "ISOC" : ((as_mic_intf_ep_attr & USB_BM_ATTRIBUTES_XFERTYPE_MASK) == USB_BM_ATTRIBUTES_XFER_BULK ? "BULK" : "Unknown"), as_mic_intf_ep_addr, as_mic_intf_ep_mps);
...
IRAM_ATTR static void _uvc_process_payload(_uvc_stream_handle_t *strmh, size_t req_len, uint8_t *payload, size_t payload_len)
{
    ...
    if (!flag_zlp)
    {
        ...
        if (payload_len >= payload[0] && (payload[0] == 12 || (payload[0] == 2 && !(payload[1] & 0x0C)) || (payload[0] == 6 && !(payload[1] & 0x08))) && !(payload[1] & 0x30)
    ...
    if (data_len >= 1)
    {
        if (strmh->got_bytes + data_len > s_usb_dev.uvc_cfg.xfer_buffer_size)
        {
            ...
            ESP_LOGW(TAG, "Transfer buffer overflow, got data=%u B, last=%u (%02x %02x...%02x %02x)", strmh->got_bytes + data_len, data_len, payload[0], payload_len > 1 ? payload[1] : 0, payload_len > 1 ? payload[payload_len - 2] : 0, payload_len > 1 ? payload[payload_len - 1] : 0);
            _uvc_drop_buffers(strmh);
#else
...
{
    if ((s_usb_dev.enabled[STREAM_UAC_MIC] && !s_usb_dev.uac->as_ifc[UAC_MIC]->not_found) || (s_usb_dev.enabled[STREAM_UAC_SPK] && !s_usb_dev.uac->as_ifc[UAC_SPK]->not_found) || (s_usb_dev.enabled[STREAM_UVC] && !s_usb_dev.uvc->vs_ifc->not_found))
    {
...
static void _usb_stream_handle_task(void *arg)
{
    ...
    _usb_stream_recover:
        ...
        EventBits_t uxBits = xEventGroupWaitBits(usb_dev->event_group_hdl, USB_STREAM_TASK_KILL_BIT | USB_STREAM_TASK_RECOVER_BIT, pdFALSE, pdFALSE, pdMS_TO_TICKS(1000));
...
static esp_err_t _uvc_uac_device_enum(bool abort_process, bool *waiting_urb_done)
{
    ...
    else if (usb_dev->enum_stage % 2)
    {                                   // Check if we are in check stage, which is always odd number
...
static void _usb_processing_task(void *arg)
{
    ...
    while (1)
    {
        ...
        if (usb_dev->state == STATE_DEVICE_ENUM && (action_bits & (ACTION_PORT_RECOVER | ACTION_PORT_DISABLE | ACTION_PIPE_DFLT_DISABLE)))
        {
        ...
        if (usb_dev->state == STATE_DEVICE_ACTIVE && (action_bits & (ACTION_PIPE_XFER_FAIL | ACTION_PIPE_DFLT_CLEAR | ACTION_PIPE_DFLT_RECOVER | ACTION_PIPE_DFLT_DISABLE)))
        {
...
esp_err_t uac_frame_size_reset(usb_stream_t stream, uint8_t ch_num, uint16_t bit_resolution, uint32_t samples_frequence)
{
    ...
    {
        if (frame_size[i].ch_num == ch_num && frame_size[i].bit_resolution == bit_resolution && (frame_size[i].samples_frequence == samples_frequence || (samples_frequence >= frame_size[i].samples_frequence_min && samples_frequence <= frame_size[i].samples_frequence_max)))
        {
            frame_index = i + 1;
            frame_found = true;
            break;
    ...
    if (frame_found && s_usb_dev.uac->ch_num[uac_stream] == ch_num && s_usb_dev.uac->bit_resolution[uac_stream] == bit_resolution && s_usb_dev.uac->samples_frequence[uac_stream] == samples_frequence)
    {
        UVC_EXIT_CRITICAL();
...
esp_err_t uvc_frame_size_reset(uint16_t frame_width, uint16_t frame_height, uint32_t frame_interval)
{
    ...
    if (frame_width && frame_height)
    {
        ...
        {
            if ((frame_width == FRAME_RESOLUTION_ANY || frame_width == frame_size[i].width) && (frame_height == FRAME_RESOLUTION_ANY || frame_height == frame_size[i].height))
            {
